...
function checkAdminAuth(req, res, next) {
    ...
    if (authHeader === ADMIN_PASSWORD) { next(); } 
...
app.get('/api/queue', (req, res) => {
    ...
    let submissionsOpen = (!dbData.extensionActive && totalSeconds < baseMaxSeconds) || (dbData.extensionActive && totalSeconds < extensionMaxSeconds);
...
app.post('/api/submit', (req, res) => {
    if (dbData.systemOnline === false) return res.status(400).json({ error: "Das Formular ist offline!" });

    const { artist, title, duration, genre, songLink } = req.body;
    if (!/spotify\.com/i.test(songLink) && !/youtube\.com|youtu\.be/i.test(songLink)) return res.status(400).json({ error: "Nur Spotify oder YouTube Links!" });
    if (dbData.songQueue.some(s => s.songLink.trim().toLowerCase() === songLink.trim().toLowerCase())) return res.status(400).json({ error: "Song-Link schon vorhanden!" });
    if (["Schlager", "Hardstyle", "Hardcore", "Metal"].includes(genre)) return res.status(400).json({ error: "Genre verboten!" });
...
app.post('/api/queue/:index/hit', checkAdminAuth, (req, res) => {
    const song = dbData.songQueue[parseInt(req.params.index)];
    ...
    if (song.isHit) dbData.historicalHits[dateStr].push({ artist: song.artist, title: song.title, genre: song.genre });
    else dbData.historicalHits[dateStr] = dbData.historicalHits[dateStr].filter(s => s.title !== song.title);
    saveToDB(); res.json({ success: true });
});
...
app.post('/api/admin/toggle-voting', checkAdminAuth, (req, res) => { dbData.votingActive = !dbData.votingActive; if(dbData.votingActive) dbData.votes = {}; saveToDB(); res.json({ success: true }); });
app.post('/api/vote', (req, res) => { dbData.votes[req.body.songIndex] = (dbData.votes[req.body.songIndex] || 0) + 1; saveToDB(); res.json({ success: true }); });
app.delete('/api/admin/halloffame/:index', checkAdminAuth, (req, res) => { dbData.hallOfFame.splice(parseInt(req.params.index), 1); saveToDB(); res.json({ success: true }); });
app.post('/api/queue/reset', checkAdminAuth, (req, res) => { dbData.songQueue = []; dbData.votes = {}; dbData.votingActive = false; dbData.extensionActive = false; saveToDB(); res.json({ success: true }); });
