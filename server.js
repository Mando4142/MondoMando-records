    ...
    try { 
        ...
        dbData.songQueue = dbData.songQueue.map(song => {
            ...
            if (!Array.isArray(song.platforms)) {
                ...
                else if (/youtube\.com|youtu\.be/i.test(song.songLink || '')) song.platforms = ['youtube'];
                else song.platforms = [];
            }
            if (!song.platform && song.platforms.length === 1) song.platform = song.platforms[0];
            if ((!song.platform || song.platform === 'other') && song.platforms.length > 1) song.platform = 'both';
            return song;
        });
...
const FEEDBACK_BUFFER_SECONDS = 60;
